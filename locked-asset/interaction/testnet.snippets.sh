...
transferNFTTokens() {
    ...
    address_to_send="$(mxpy wallet bech32 --decode $1)"
...
deployLockedAssetContract() {
    ...
    ADDRESS=$(mxpy data parse --file="deploy-locked-asset-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeLockedAssetContract() {
    ...
    mxpy --verbose contract upgrade $LOCKED_ASSET_FACTORY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-price=1499999999 \
        --gas-limit=1499999999 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
registerLockedAssetToken() {
    ...
    mxpy --verbose contract call ${LOCKED_ASSET_FACTORY_ADDRESS} --recall-nonce \
          --pem=${WALLET_PEM} \
          --proxy=${PROXY} --chain=${CHAIN_ID} \
          --gas-limit=200000000 \
...
setLocalRolesLockedAssetToken() {
    decoded_address="0x$(mxpy wallet bech32 --decode $1)"
...
#   $1 = Address
whitelist() {
    sc_address="0x$(mxpy wallet bech32 --decode $1)"
...
deployProxyContract() {
    ...
    ADDRESS=$(mxpy data parse --file="deploy-proxy-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeProxyContract() {
    ...
    mxpy --verbose contract upgrade $PROXY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=300000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --metadata-payable \
...
registerProxyPair() {
    ...
    mxpy --verbose contract call $PROXY_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
transferNFTTokensProxy() {
    ...
    pair_address="0x$(mxpy wallet bech32 --decode $1)"
    proxy_address="0x$(mxpy wallet bech32 --decode $PROXY_ADDRESS)"
...
deployDistributionContract() {
    locked_asset_address="0x$(mxpy wallet bech32 --decode $LOCKED_ASSET_FACTORY_ADDRESS)"
    asset_token="0x$(echo -n $1 | xxd -p -u | tr -d '\n')"
    ...
    ADDRESS=$(mxpy data parse --file="deploy-distribution-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeDistributionContract() {
    ...
    mxpy --verbose contract upgrade $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-price=1499999999 \
        --gas-limit=1499999999 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
startGlobalOperation() {
    mxpy --verbose contract call $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=20000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
setPerUserDistributedLockedAssets() {
    user_address="0x$(mxpy wallet bech32 --decode $2)"
    mxpy --verbose contract call $DISTRIBUTION_ADDRESS --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=50000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
...
getTemporaryFundsProxy() {
    user_address="0x$(mxpy wallet bech32 --decode $1)"
    asset_token="0x$(echo -n $2 | xxd -p -u | tr -d '\n')"

    mxpy --verbose contract query $PROXY_ADDRESS \
        --proxy=${PROXY} \
...
# params:
getLockedTokenId() {
    mxpy --verbose contract query $LOCKED_ASSET_FACTORY_ADDRESS \
        --proxy=${PROXY} \
...
getLastCommunityDistributionUnlockMilestones() {
    mxpy --verbose contract query $DISTRIBUTION_ADDRESS \
        --proxy=${PROXY} \
